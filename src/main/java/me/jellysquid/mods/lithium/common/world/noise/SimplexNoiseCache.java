...
public class SimplexNoiseCache {
    ...
    private final SimplexNoise sampler;

    public SimplexNoiseCache(SimplexNoise sampler) {
    ...
    private float getDistanceFactor(int x, int z) {
        ...
        if (distanceFromOriginSq > 64 * 64) {
            ...
            if (this.sampler.getValue(x, z) < -0.9) {
                ...
                value = (Mth.abs(x) * 3439.0F + Mth.abs(z) * 147.0F) % 13.0F + 9.0F;
            }
        }
    ...
    public float getNoiseAt(int x, int z) {
        ...
        float density = ISLAND_RADIUS - Mth.sqrt(x * x + z * z) * 8.0F;
        if (density >= MAX) {
            return MAX;
        }
        ...
        for (int offsetX = -12; offsetX <= 12; ++offsetX) {
            for (int offsetZ = -12; offsetZ <= 12; ++offsetZ) {
                ...
                if (distanceFactor != -1.0F) {
                    ...
                    float scaledDistance = Mth.sqrt(deltaX * deltaX + deltaZ * deltaZ) * distanceFactor;
