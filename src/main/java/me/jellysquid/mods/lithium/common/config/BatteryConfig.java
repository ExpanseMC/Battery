...
public class BatteryConfig {
    private static final Logger LOGGER = LogManager.getLogger("BatteryConfig");

    private static final String JSON_KEY_BATTERY_OPTIONS = "battery:options";
    ...
    private void applyModOverrides() {
        // TODO(doot): Too early for sponge atm
//        for (PluginContainer container : Sponge.pluginManager().plugins()) {
//            final PluginMetadata meta = container.metadata();
//            final Map<String, Object> extra = meta.extraMetadata();
//
//            final Object options = extra.get(JSON_KEY_BATTERY_OPTIONS);
//            if (!(options instanceof Map)) {
//                LOGGER.warn("Plugin '{}' contains invalid Battery option overrides, ignoring", meta.id());
//            }
//            final Map<String, Object> optionMap = (Map<String, Object>) options;
//
//            for (final Map.Entry<String, Object> entry : optionMap.entrySet()) {
//                this.applyModOverride(meta, entry.getKey(), entry.getValue());
//            }
//        }
    }

    private void applyModOverride(PluginMetadata meta, String name, Object value) {
        Option option = this.options.get(name);

        if (option == null) {
            LOGGER.warn("Plugin '{}' attempted to override option '{}', which doesn't exist, ignoring", meta.id(), name);
            return;
        }

        if (!(value instanceof Boolean)) {
            LOGGER.warn("Plugin '{}' attempted to override option '{}' with an invalid value, ignoring", meta.id(), name);
            return;
        }
        ...
        if (!enabled || option.isEnabled() || option.getDefiningMods().isEmpty()) {
            option.addModOverride(enabled, meta.id());
        }
    }
     ...
     */
    public static BatteryConfig load(File file) {
    ...
    private static void writeDefaultConfig(File file) throws IOException {
        ...
        try (Writer writer = new FileWriter(file)) {
            writer.write("# This is the configuration file for Battery.\n");
