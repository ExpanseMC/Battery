...
public class LithiumServerTickScheduler<T> extends ServerTickList<T> {
    ...
    public void copy(BoundingBox box, BlockPos pos) {
        ...
        for (TickNextTickData<T> tick : list) {
            this.addTickData(new TickNextTickData<>(tick.pos.offset(pos), tick.getType(), tick.triggerTick, tick.priority));
        }
    }

    @Override
    public void scheduleTick(BlockPos pos, T obj, int delay, TickPriority priority) {
        if (!this.ignore.test(obj)) {
            this.addTickData(new TickNextTickData<>(pos, obj, (long) delay + this.world.getGameTime(), priority));
        }
    }
     ...
     */
    private void addTickData(TickNextTickData<T> tick) {
